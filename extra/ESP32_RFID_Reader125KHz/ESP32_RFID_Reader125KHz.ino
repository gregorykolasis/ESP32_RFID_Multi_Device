...
int rfidDemod = 26; // input - demod pin from RDM6300, pullup high
int rfidClock = 25; // output - clock pin to RDM6300
...
static bool findHeader() {
  ...
  Serial.printf("No %s header found\n", codeStr[encodeFDX]);
  return false;
}
...
static bool parseBitBufferEM(uint8_t currIndex) {
  ...
  for (uint8_t count = 0; count < 2; count ++) {
    if ( !parseNibble(offset + 9 + 5 * count, &nibble)) {
      //Serial.printf(" - Wrong parity for card facility, nibble %d \n", count);
      return false;
    }
  ...
  for (uint8_t count = 0; count < 8; count ++) {
    if ( ! parseNibble(offset + 19 + 5 * count, &nibble)) {
      Serial.printf(" EM Wrong parity for card UID, nibble %d \n", count);
      return false;
    }
  ...
  for (uint8_t i = 0; i < 4; i++) {
    ...
    if ((columnSums[i] & 1) != theBit) {
      Serial.printf(" - Wrong parity for column %d \n", i);
      return false;
    }
    index++;
  }
...
static bool parseBitBufferFDX(uint8_t currIndex) {
  ...
  if (!theBit) {
    Serial.printf("FDX-B header not terminated");
    return false;
  } 
  headerPos = currIndex - 10;
  Serial.printf("%s header start at: %d", codeStr[encodeFDX], headerPos);
  ...
  for (uint8_t i = offset; i < offset+53; i++) {
    ...
    if (count < 8) {
      ...
      if (!theBit) {
        Serial.printf("FDX-B control bit wrong");
        return false;
      } else count = 0;
    }
  }
  ...
  Serial.printf("%s", codeStr[encodeFDX]);
  return true;
...
void rfidRead() {
  ...
  if (captured) {
    ...
    if (!haveTag && !encodeFDX) {
      ...
      //Serial.println("Retry after logic inversion");
...
void setup() {
  ...
  Serial.println("125kHz PWM signal is being generated...");
}
